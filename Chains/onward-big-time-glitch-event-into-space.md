...
**Clock them together.** Run the **Digitakt 2 as the single clock master** sending MIDI clock to both boxes (via the CB MIDIbox/TRS). Onward syncs its Glitch stutter timing to clock (SIZE = subdivision); Big Time locks TIME via CC54 subdivision. Because they share one grid, every glitch grain lands on a tap and every tap lands on the grid — that's the stutter-on-stutter lock. Pick **ONE** master and never loop clock back. Signal is mono-in/stereo-out friendly: a mono source into Onward, stereo out of Onward into Big Time's stereo ins, stereo to tape.
