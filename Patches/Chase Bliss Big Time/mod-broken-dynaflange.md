...
- ⚠️ PROVISIONAL on envelope polarity: the exact polarity of Env (whether the sweep opens on attack-up or releases on decay-down) is firmware-determined and currently unverified in the corpus — so the "fills the gaps between notes / breathing" behavior is the intended character, but the precise attack-vs-release feel may need dialing in by ear once confirmed on hardware.
...
- Env-polarity caveat (firmware-determined, unverified): `research/bloops/2026-06-19-chase-bliss-big-time.md` (Follow-up questions).
